const logger = require("debug")("app:co-op");
const config = require("../config");
const tools = require("../utils/tools");
const MidiHelper = require("../utils/midiHelper");
// 读取midi文件列表
const midiFileList = MidiHelper.listAllMidi();
...
const preInterceptor = (socket, next) => {
	const { name, mode = "co-op", room, midiName } = socket.handshake.query;
	...
	// midi校验
	if (!midiName || !midiFileList.includes(midiName)) {
		return next(new Error("The midi name is not exist!"));
	}
...
const commonInterceptor = (
	...
	const { name, mode = "co-op", room } = socket.handshake.query;
