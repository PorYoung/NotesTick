<template>
  <el-container v-loading.fullscreen.lock="preloading" class="main-wrapper">
    ...
    <el-main>
      <noteRainFull ref="noteRain" />
    </el-main>
    <el-footer>
      <el-row :gutter="2">
        <el-col :span="6">
          <button class="btn" :disabled="this.started" @click="toReady">
        ...
        <el-col :span="6">
          <button
            class="btn"
            :disabled="
              this.started || !this.ready || this.userId !== this.roomCreator
            "
            @click="toStart"
          >
            {{
    ...
    >
      <el-form>
        <el-form-item>
          <el-input placeholder="如：admin" v-model:value="name">
            <template v-slot:prepend>请输入您的名字</template>
          </el-input>
        </el-form-item>
      </el-form>
      <template v-slot:footer>
        <span class="dialog-footer">
          <el-button type="primary" @click="confirmInputName">确 定</el-button>
        </span>
      </template>
    </el-dialog>
...
import noteRainFull from "./components/note-rain-full.vue";
...
export default {
  ...
  mounted() {
    ...
    $on(this.$refs.noteRain, "finished", () => {
      this.finished = true;
      this.progress = 100;
      this.socket.emit("GAME_OVER");
    });
    $on(this.$refs.noteRain, "progress", (now) => {
      const last = this.midiNotes.slice(-1).pop();
      this.progress = last
        ? ((now - this.startTime) / (this.blankTime + last.time * 1000)) * 100
        : 0;
    });
  },
  methods: {
    confirmInputName() {
      ...
      if (name != "") {
        const { room, midi, velocity, auto, maxKeys } = this.$route.query;
        this.name = name;
    ...
    onBroadcastNote(data) {
      ...
      $emit(this.$refs.noteRain, "remotePressed", data.note);
    },
    onReleaseNote(data) {
      // {
      //     note /* 音符 */,
      ...
      $emit(this.$refs.noteRain, "remoteReleased", data.note);
    },
    onStartEvent(data) {
      this.preloading = false;
      this.started = true;
    ...
    onKeyDown(e) {
      ...
      if (e.key in this.keyNotesMap && !this.keyPressed.has(e.key)) {
        ...
        $emit(this.$refs.noteRain, "handleKeyDown", note);
    ...
    onKeyUp(e) {
      ...
      if (e.key in this.keyNotesMap && this.keyPressed.has(e.key)) {
        const note = this.keyNotesMap[e.key];
        $emit(this.$refs.noteRain, "handleKeyUp", note);
    ...
    cleanPlayer() {
      $emit(this.$refs.noteRain, "stopRain");
    ...
    handlePlay() {
      ...
      if (!this.isPlay) {
        ...
        $emit(this.$refs.noteRain, "continueRain");
        // 播放notes
        this.playCurrentNotesTimer(
          this.midiNotes,
          this.blankTime,
